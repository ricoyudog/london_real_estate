...
| TC-04 Future supply pipeline | **Engineering PASS:** production/discovery lane separation prevents unapproved planning evidence from becoming canonical. [`tests/test_supervisor.py`](../tests/test_supervisor.py), [`tests/test_datasource_catalog.py`](../tests/test_datasource_catalog.py) | **Product BLOCKED:** `pld.applications_search` and `pld.application` are blocked for licence and retention; there is no approved project-level City supply pipeline. | **Data-governance owner:** obtain PLD approval. **Supply-data owner:** bind a versioned collector/parser, project-status taxonomy, and coverage/backfill policy before testing annual supply and pre-let share. |
